...
-- both follow-rows exist) with an explicit request -> accept flow, per
-- product decision. follows is dropped outright: nothing else in the app
-- reads it (confirmed — only lib/social.ts's follow/unfollow/getFollowStatus
-- family, all rewritten alongside this migration).
...
-- Plain RLS can't express "auto-accept the reverse pending request" or the
-- blocked/already-friends checks, so INSERT is closed to the client entirely
-- (revoked below) and send_friend_request() is the only path in.
...
-- or block_user(), all security definer.
...
-- is also the only place friend_requests rows are ever inserted.
...
create policy "message only friends, and not if blocked"
