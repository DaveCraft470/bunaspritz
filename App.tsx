  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <KeyboardAvoidingView style={styles.page} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <ScrollView style={styles.chatScroller} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chatList}>
          {chats.map((chat) => (
            <Pressable key={chat.id} onPress={() => setActiveChat(chat.id)} style={[styles.chatCard, chat.id === activeChat && styles.chatCardActive]}>
              ...
              <View style={styles.groupNameRow}><Text numberOfLines={1} style={styles.groupName}>{chat.title}</Text></View>
            </Pressable>
          ))}
        </ScrollView>
        ...
        <View style={styles.composer}>
          ...
          <Pressable style={styles.voice} accessibilityLabel="Înregistrează mesaj vocal"><Text style={styles.voiceText}>🎤</Text></Pressable>
...
const styles = StyleSheet.create({
  ...
  emoji: { fontSize: 18 },
  chatCopy: { flex: 1 },
  chatTitle: { color: '#0B3D20', fontSize: 13, fontWeight: '800' },
  chatDetail: { color: '#64806D', fontSize: 11, marginTop: 3 },
  ...
  composer: { marginHorizontal: 16, marginBottom: 14, paddingVertical: 7, paddingHorizontal: 7, borderRadius: 22, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#CDEFD8', flexDirection: 'row', alignItems: 'center', gap: 7 },
