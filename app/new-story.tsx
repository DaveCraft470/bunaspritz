...
export default function NewStory() {
  ...
  async function post() {
    ...
    setPosting(false);
    if (!story) {
      Alert.alert('Nu am putut publica', 'Nu am putut încărca imaginea. Încearcă din nou.');
      return;
    }
    medium();
    router.back();
        ...
        <AnimatedPressable onPress={pickMedia} style={[styles.mediaCard, { backgroundColor: theme.surface, borderColor: theme.border }]}> 
          {mediaUri ? <Image source={{ uri: mediaUri }} style={styles.preview} resizeMode="cover" /> : (
            <View style={styles.mediaEmpty}>
              ...
              <Text style={[styles.mediaHint, { color: theme.textSecondary }]}>Story-ul dispare automat după 24 de ore.</Text>
            </View>
          )}
        </AnimatedPressable>
