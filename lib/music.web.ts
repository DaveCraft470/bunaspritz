const queryCache = new Map<string, SongCatalogItem[]>();
...
const MUSIC_PROXY_URL = process.env.EXPO_PUBLIC_DEEZER_PROXY_URL ?? 'http://localhost:3002';
...
export async function searchMusic(query: string): Promise<SongCatalogItem[]> {
  ...
  try {
    const response = await fetch(`${MUSIC_PROXY_URL.replace(/\/$/, '')}/api/music/search?q=${encodeURIComponent(query.trim())}`, {
    ...
    queryCache.set(normalized, songs);
    return songs;
  } catch (error) {
...
export function normalizeDeezerResult(result: DeezerTrack): SongCatalogItem | null {
  ...
  return {
    ...
    coverUrl: result.album?.cover_xl || result.album?.cover_big || result.album?.cover_medium,
